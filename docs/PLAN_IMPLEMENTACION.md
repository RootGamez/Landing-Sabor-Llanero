...
- **Pendiente (2026-09-25) — alternativa evaluada, no implementada**: el usuario preguntó si se
  puede evitar el requisito de Workers Paid usando SMTP directo desde la API. Conclusión: SMTP
  tradicional (sockets TCP + TLS manual) es mal fit para Workers — sin librerías maduras, frágil.
  La alternativa real es un proveedor con API HTTP, llamado por `fetch()` igual que cualquier
  otra API externa (no necesita el binding `send_email` ni Workers Paid). Recomendado: **Resend**
  (tiene tutorial oficial para Cloudflare Workers, free tier: 3,000 emails/mes, 100/día, 3
  dominios verificados). Si se decide desbloquear P1.3 sin esperar el upgrade a Workers Paid, el
  camino sería: crear cuenta Resend, verificar dominio (SPF/DKIM, igual que con Email Sending
  nativo), reemplazar `env.EMAIL.send(...)` por un `fetch('https://api.resend.com/emails', ...)`
  con `RESEND_API_KEY` como secret. Queda documentado como opción, **no se implementa** hasta que
...
| 2026-09-25 | Incidente de producción | `POST /api/customers/register` devolvía 500 genérico en producción (`api.saborllanero.online`). Diagnóstico: las migraciones `0003_password_reset_tokens.sql` y `0004_loyalty.sql` (toda la Parte 2: `customers`, `orders`, `order_items`, `points_ledger`, `rewards`, `reward_redemptions`, `raffle_entries`, `raffle_draws`, `loyalty_config`, más `password_reset_tokens` de P1.2) se habían construido y probado solo contra D1 **local** durante toda la sesión — nunca se corrió `wrangler d1 migrations apply --env production --remote` contra la D1 real. Confirmado listando tablas remotas (`SELECT name FROM sqlite_master...`): faltaban todas las tablas nuevas. Corregido aplicando ambas migraciones a producción (aditivas, `CREATE TABLE IF NOT EXISTS`, sin riesgo para las tablas existentes) y re-verificado con un registro de prueba real (`201 Created`, luego borrado de la DB). **Fix estructural** para que no vuelva a pasar: `apps/api/package.json`, script `deploy` ahora encadena `wrangler d1 migrations apply --env production --remote && wrangler deploy --env production` — cada `make deploy-api` aplica automáticamente cualquier migración pendiente antes de desplegar el código nuevo (idempotente: si no hay migraciones pendientes, es un no-op). Separadamente, a pedido del usuario, se agregó a P1.3 una nota **pendiente, no implementada**: usar un proveedor HTTP (Resend) como alternativa a Cloudflare Email Sending nativo para no depender de Workers Paid. | Ninguno respecto del código de la Parte 2 en sí (el bug era puramente de despliegue/operación, no de lógica); el `Makefile` y su comentario de orden de despliegue (`secrets -> db-migrate-remote -> db-seed-remote -> ...`) siguen documentando el bootstrap inicial paso a paso, pero `db-migrate-remote` como paso manual separado ya no es estrictamente necesario para redeploys posteriores al estar ahora dentro de `deploy-api` |
